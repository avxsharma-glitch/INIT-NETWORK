...
import { ArrowLeft, ExternalLink, GitBranch, X as XIcon, Globe } from "lucide-react";
import { currentUser } from "@/data/mock";
import { statusTextColors } from "@/data/mock";
...
function BuilderProfile({ builder }: { builder: Builder }) {
  const projects = getProjectsForBuilder(builder.id);
      ...
      <div className="p-6 rounded-2xl bg-[#141414] border border-[#1F1F1F]">
        <div className="flex flex-wrap gap-6 items-start">
          ...
          <div className="flex-1 min-w-0">
            ...
            <p className="text-sm text-[#878787] mt-3 leading-relaxed max-w-xl">{builder.bio}</p>
                ...
                {builder.achievementIds.map((aid) => (
                  <div key={aid} className="px-4 py-3 rounded-xl bg-[#141414] border border-[#1F1F1F] flex items-center gap-3">
                    <span className="text-lg shrink-0">
                      {aid === "ach1" ? "🚀" : aid === "ach2" ? "🔗" : aid === "ach3" ? "🏛️" : "🌐"}
                    </span>
                    <div>
                      <p className="text-xs font-medium text-[#ccc]">
                        {aid === "ach1" ? "First Ship" : aid === "ach2" ? "Connector" : aid === "ach3" ? "Chapter Founder" : "Open Source"}
                      </p>
                    </div>
                  </div>
                ))}
