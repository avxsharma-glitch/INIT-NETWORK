...
import { mockTeams, mockProjects, getBuilderById, type Team } from "@/data/mock";
...
function TeamDetail({ team }: { team: Team }) {
  ...
  return (
    <div className="space-y-8">
      <Link href="/app/teams" className="flex items-center gap-2 text-xs font-mono text-[#555] hover:text-[#878787] transition-colors w-fit">
                    ...
                    <div className="flex items-center gap-3 p-4 rounded-xl bg-[#141414] border border-[#1F1F1F] hover:border-[#2A2A2A] transition-colors cursor-pointer group">
                      ...
                      <div className="flex flex-wrap gap-1 justify-end">
                        {builder.skills.slice(0, 2).map((s) => (
                          <span key={s.name} className="text-[8px] font-mono text-[#555] border border-[#1F1F1F] rounded px-1.5 py-0.5">{s.name}</span>
                        ))}
                      </div>
                    </div>
                  </Link>
            ...
            <section>
              ...
              <div className="space-y-2">
                {team.openRoles.map((role) => (
                  <div key={role}
                    className="flex items-center justify-between p-3 rounded-xl bg-[#141414] border border-[#1F1F1F] hover:border-[#89AACC]/30 transition-colors group cursor-pointer">
                    <span className="text-sm text-[#ccc]">{role}</span>
                    <button className="text-[9px] font-mono text-[#89AACC] border border-[#89AACC]/30 bg-[#89AACC]/5 rounded px-2 py-0.5 hover:bg-[#89AACC]/10 transition-colors opacity-0 group-hover:opacity-100">
                      APPLY
                    </button>
                  </div>
                ))}
          ...
          <section>
            ...
            <div className="bg-[#141414] border border-[#1F1F1F] rounded-xl overflow-hidden">
              {[
                ...
                <div key={d.label} className={`px-4 py-3 flex justify-between ${i < 2 ? "border-b border-[#1F1F1F]" : ""}`}>
  ...
  return (
    <div className="space-y-8">
      <div>
        ...
        <p className="text-sm text-[#555] mt-1">Find teams to join and projects to contribute to.</p>
      </div>
              ...
              <div className="p-5 rounded-xl bg-[#141414] border border-[#1F1F1F] hover:border-[#2A2A2A] transition-colors cursor-pointer group h-full flex flex-col">
                ...
                {team.openRoles.length > 0 && (
                  <div className="mt-4 pt-4 border-t border-[#1F1F1F]">
                    <p className="text-[9px] font-mono text-[#555] mb-2">OPEN ROLES</p>
                    <div className="flex flex-wrap gap-1">
                      {team.openRoles.map((r) => (
                        <span key={r} className="text-[9px] font-mono text-[#89AACC] border border-[#89AACC]/20 bg-[#89AACC]/5 rounded px-1.5 py-0.5">{r}</span>
                      ))}
                    </div>
                  </div>
                )}
                <div className="mt-4 flex items-center justify-between">
                  ...
                  <span className="text-[9px] font-mono text-[#444]">{team.memberIds.length} MEMBERS</span>
                </div>
              </div>
            </Link>
          );
  ...
  if (!team) {
    return (
      <div className="flex flex-col items-center justify-center h-64 text-[#444] font-mono text-sm gap-4">
        <p>Team not found.</p>
        <Link href="/app/teams" className="text-[#89AACC] text-xs hover:underline">← Back to teams</Link>
      </div>
    );
  }
